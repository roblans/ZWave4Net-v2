...
namespace ZWave4Net
{
    public class ZWaveController
    {
        ...
        public async Task Open()
        {
            ...
            var getVersion = await Channel.Send<PayloadBytes>(new ControllerRequest(Function.GetVersion));
            using (var reader = new PayloadReader(getVersion))
            {
                Version = reader.ReadString();
            }

            var memoryGetId = await Channel.Send<PayloadBytes>(new ControllerRequest(Function.MemoryGetId));
            ...
            var discoveryNodes = await Channel.Send<PayloadBytes>(new ControllerRequest(Function.DiscoveryNodes));
